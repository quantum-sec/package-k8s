output "generation" {
  ...
  value       = kubernetes_deployment.deployment.metadata[0].generation
}
...
output "resource_version" {
  ...
  value       = kubernetes_deployment.deployment.metadata[0].resource_version
}

output "self_link" {
  description = "A URL representing this deployment."
  value       = kubernetes_deployment.deployment.metadata[0].self_link
}

output "uid" {
  description = "The unique identifier of this deployment."
  value       = kubernetes_deployment.deployment.metadata[0].uid
}
